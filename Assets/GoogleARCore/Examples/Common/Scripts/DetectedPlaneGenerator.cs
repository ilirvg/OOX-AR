...
namespace GoogleARCore.Examples.Common {
    ...
    public class DetectedPlaneGenerator : MonoBehaviour {
        public static DetectedPlaneGenerator instance = null; //ADDED
        public List<GameObject> PLANES = new List<GameObject>(); //ADDED
        /// <summary>
        ...
        private List<DetectedPlane> allPlanes = new List<DetectedPlane>();//ADDED
        ...
        public void Update() {
            ...
            for (int i = 0; i < m_NewPlanes.Count; i++) {
                ...
                PLANES.Add(planeObject); //ADDED
                planeObject.GetComponent<DetectedPlaneVisualizer>().Initialize(m_NewPlanes[i]);
                CheckPlanes(m_NewPlanes[i]);
            }
        }

        //ADDED everyting below this comment
        public void CheckPlane(TrackableHit hit) {
            foreach (DetectedPlane d in allPlanes) {

                if (d == hit.Trackable) {
                    if (d.PlaneType == DetectedPlaneType.HorizontalDownwardFacing || d.PlaneType == DetectedPlaneType.HorizontalUpwardFacing) {
                        DBManager.planeType = 0;
                    }
                    else {
                        DBManager.planeType = 1;
                    }
                }
            }
        }
        public void CheckPlanes(DetectedPlane d) {
            allPlanes.Add(d);
        }
    }
}
