...
#endif
#ifdef Enable_Bloom
				fixed4 c = 2.0f * i.color * _TintColor * tex2D(_MainTex, i.uv_MainTex) * (exp(_EmissionGain * 5.0f) * _GlobalLightEstimation);
#else
				fixed4 c = 2.0f * i.color * _TintColor * tex2D(_MainTex, i.uv_MainTex) * _GlobalLightEstimation;
#endif
#ifdef Enable_AlphaMask
